...
int tongduong(int n,int m,int a[50][50]){
	int s=0;
	for(int i=0;i<m;i++){
		for(int j=0;j<n;j++){
			if(a[i][j]>0)
...
int tongam(int n,int m,int a[50][50]){
	int tong=0;
	for(int i=0;i<m;i++){
		for(int j=0;j<n;j++){
			if(a[i][j]<0)
...
int main(){
	...
	cout<<"tong cac pt duong trong mang: "<<tongduong(n,m,a)<<endl;
	cout<<"tong cac pt am trong mang: "<<tongam(n,m,a);
	return 0;
}